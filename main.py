...
class Application(tk.Tk, Configure_widjets):
    def __init__(self):
        ...
        self.title('Нарузка на ЦП и ОЗУ')
        self.cpu = CpuBar()
        self.run_set_ui()
    def none_func(self):
        print("DOC")
        webbrowser.open('https://psutil.readthedocs.io/en/latest/', new=2)
    def run_set_ui(self):
        self.set_ui()
    ...
    def set_ui(self):
        ...
        self.bar2 = ttk.LabelFrame(self, text='Руководство')
        ...
        ttk.Button(self.bar2, text='двигать',
                    command=self.configure_win).pack(side=tk.LEFT)
        ttk.Button(self.bar2, text='документация psutil', command=self.none_func).pack(side=tk.LEFT)
    ...
    def make_bar_cpu_usage(self):
        ttk.Label(self.bar,
                text=f'физические ядра: {self.cpu.cpu_count}, логические ядра: {self.cpu.cpu_count_logical}',
    ...
    def make_minimal_win(self):
        ...
        ttk.Button(self, text='полн', width=5,
                    command=self.make_full_win).pack(side=tk.RIGHT)
        ttk.Button(self, text='двиг', width=5,
